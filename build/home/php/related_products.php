...
foreach ($_SESSION['carrito'] as $key => $value) {
    ...
    array_push($array_categories, $row['IdProducto']);
}
$array = array_unique($array_categories);
$array_products_related = array();
for ($i=0; $i < count($array); $i++) {
    $query1 = "SELECT * FROM Productos p INNER JOIN Marcas m ON m.IdMarca = p.Marcas_IdMarca WHERE p.Estatus = 'Activo' AND p.Stock > 0 AND p.Categorias_IdCategoria = '".$array[$i]."' ORDER BY RAND() LIMIT 4";
